...
class IsilogProvider extends AbstractProvider {
    ...
    */
    protected function saveConfigExtra() {
        ...
        $this->_save_config['simple']['centreoncat'] = $this->_submitted_config['centreoncat'];
        $this->_save_config['simple']['centreonservice'] = $this->_submitted_config['centreonservice'];
        $this->_save_config['simple']['centreonteam'] = $this->_submitted_config['centreonteam'];
        $this->_save_config['simple']['centreonsite'] = $this->_submitted_config['centreonsite'];
        $this->_save_config['simple']['centreonou'] = $this->_submitted_config['centreonou'];
        $this->_save_config['simple']['centreonuser'] = $this->_submitted_config['centreonuser'];
        $this->_save_config['simple']['centreonimpact'] = $this->_submitted_config['centreonimpact'];
        $this->_save_config['simple']['centreonurgency'] = $this->_submitted_config['centreonurgency'];
        $this->_save_config['simple']['centreonothers'] = $this->_submitted_config['centreonothers'];
    ...
    */
    protected function assignIsilogCategory($entry, &$groups_order, &$groups) {
        ...
        $result = array();
        $xmlResults=$listCategories->children('s', true)->Body->children()->IsiGetQueryResultResponse->IsiGetQueryResultResult
    ...
    */
    protected function assignIsilogService($entry, &$groups_order, &$groups) {
        ...
        $xmlResults=$listServices->children('s', true)->Body->children()->IsiGetQueryResultResponse->IsiGetQueryResultResult
            ->Objects->children('b', true)->anyType->children()->IsiWsEntity;
        foreach ($xmlResults as $xmlResult) {
    ...
    */
    protected function assignIsilogTeam($entry, &$groups_order, &$groups) {
        ...
        $xmlResults=$listTeams->children('s', true)->Body->children()->IsiGetQueryResultResponse->IsiGetQueryResultResult
            ->Objects->children('b', true)->anyType->children()->IsiWsEntity;
        foreach ($xmlResults as $xmlResult) {
    ...
    */
    protected function assignIsilogOU($entry, &$groups_order, &$groups) {
        ...
        $xmlResults=$listOU->children('s', true)->Body->children()->IsiGetQueryResultResponse->IsiGetQueryResultResult
            ->Objects->children('b', true)->anyType->children()->IsiWsEntity;
        foreach ($xmlResults as $xmlResult) {
    ...
    */
    protected function assignIsilogSite($entry, &$groups_order, &$groups) {
        ...
        $xmlResults=$listSites->children('s', true)->Body->children()->IsiGetQueryResultResponse->IsiGetQueryResultResult
            ->Objects->children('b', true)->anyType->children()->IsiWsEntity;
        foreach ($xmlResults as $xmlResult) {
            ...
            // if we do have have a filter, we make sure that the match the filter, if so, we put the name and the id
            // of the site inside the result array
    ...
    */
    protected function assignIsilogUser($entry, &$groups_order, &$groups) {
        ...
        $xmlResults=$listUsers->children('s', true)->Body->children()->IsiGetQueryResultResponse->IsiGetQueryResultResult
            ->Objects->children('b', true)->anyType->children()->IsiWsEntity;
        foreach ($xmlResults as $xmlResult) {
    ...
    */
    protected function assignIsilogImpact($entry, &$groups_order, &$groups) {
        ...
        $xmlResults=$listImpacts->children('s', true)->Body->children()->IsiGetQueryResultResponse->IsiGetQueryResultResult
            ->Objects->children('b', true)->anyType->children()->IsiWsEntity;
        foreach ($xmlResults as $xmlResult) {
            ...
            // if we do have have a filter, we make sure that the match the filter, if so, we put the name and the id
            // of the impact inside the result array
    ...
    */
    protected function assignIsilogUrgency($entry, &$groups_order, &$groups) {
        ...
        $xmlResults=$listUrgencies->children('s', true)->Body->children()->IsiGetQueryResultResponse->IsiGetQueryResultResult
            ->Objects->children('b', true)->anyType->children()->IsiWsEntity;
        foreach ($xmlResults as $xmlResult) {
            ...
            // if we do have have a filter, we make sure that the match the filter, if so, we put the name and the id
            // of the urgency inside the result array
    ...
    */
    protected function assignIsilogQualifier($entry, &$groups_order, &$groups) {
        ...
        $xmlResults=$listQualifiers->children('s', true)->Body->children()->IsiGetQueryResultResponse->IsiGetQueryResultResult
            ->Objects->children('b', true)->anyType->children()->IsiWsEntity;
        foreach ($xmlResults as $xmlResult) {
            $isQualifier = false;
            ...
            if ($isQualifier) {
                ...
                // if we do have have a filter, we make sure that the match the filter, if so, we put the name and the id
                // of the qualifier inside the result array
                if (preg_match('/' . $entry['Filter'] . '/', $qualifierName)) {
                    $result[$qualiferId] = $this->to_utf8($qualifierName);
                }
            }
        }
    ...
    */
    protected function assignIsilogOrigin($entry, &$groups_order, &$groups) {
        ...
        $xmlResults=$listOrigins->children('s', true)->Body->children()->IsiGetQueryResultResponse->IsiGetQueryResultResult
            ->Objects->children('b', true)->anyType->children()->IsiWsEntity;
        foreach ($xmlResults as $xmlResult) {
            $isOrigin = false;
            ...
            if ($isOrigin) {
                ...
                // if we do have have a filter, we make sure that the match the filter, if so, we put the name and the id
                // of the origin inside the result array
    ...
    */
    static public function test($info) {
        ...
        $soapInfo['envelope'] = '<soap:Envelope xmlns:s="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:wsa="http://wwww.w3.org/2005/08/addressing" xmlns:isil="http://isilog.fr/" >
                <soap:Header xmlns:wsa="http://www.w3.org/2005/08/addressing">' . $soapHeader . '
                <wsa:Action>http://isilog.fr/IIsiQueryService/IsiGetQueryResult</wsa:Action><wsa:To>https://demo.iws-saas.fr/IsilogWebSystem/WebServices/IsiQueryService.svc</wsa:To></soap:Header>
                <soap:Body>
    ...
    */
    protected function createTicket($ticketArguments) {
        ...
        $soapInfo = array(
            ...
            'envelope' => '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:isil="http://isilog.fr">
                        ...
                        <isil:IsiAddAndGetCall>
                            <isil:pIsiCallEntity>
                                <isil:IsiFields>
                                    ...
                                    </isil:IsiWsDataField>' . $ticketFields . '
                                </isil:IsiFields>
                            </isil:pIsiCallEntity>
                        </isil:IsiAddAndGetCall>
                    </soapenv:Body>
    ...
    */
    protected function getSoapEnvelope($webserviceName, $numberOfResult) {
        $envelope = '<soap:Envelope xmlns:s="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:wsa="http://wwww.w3.org/2005/08/addressing" xmlns:isil="http://isilog.fr/" >
    ...
    */
    protected function closeTicketIsilog($ticketId) {
        ...
        $soapInfo = array(
            ...
            'envelope' => '<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:isil="http://isilog.fr">
